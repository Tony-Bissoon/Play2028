...
public class Board {
    ...
    public void addRandomTile() {
        // WRITE YOUR CODE HERE
        BoardSpot tile = openSpaces.get(StdRandom.uniform(0, openSpaces.size()));

        double value = StdRandom.uniform(0.0, 1.0);
        if(value < 0.1){
            gameBoard[tile.getRow()][tile.getCol()] = 4;
        }
        else{
            gameBoard[tile.getRow()][tile.getCol()] =2; 
        }
        ...
        for(int row = 0; row < gameBoard.length; row++ ){
            ...
            for(int col = 0; col < gameBoard[row].length; col++){
                if(gameBoard[row][col] != 0){
                    gameBoard[row][index++] = gameBoard[row][col];


                }
            }
            for(int col = index; col < gameBoard[row].length; col++){
                gameBoard[row][col] =0;
            }
    ...
    public void flipRows() {
        ...
        for(int row = 0; row < gameBoard.length; row++){
            int left = 0;
            int right = gameBoard[row].length-1; 
            while(left < right){
                int temp = gameBoard[row][left];
                gameBoard[row][left] = gameBoard[row][right];
                gameBoard[row][right] = temp;
                left ++;
                right--;
